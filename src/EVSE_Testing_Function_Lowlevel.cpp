...
/*
  return  true  -> Line Voltage not detected
          false -> Line Voltage detected
*/
...
/*
  return  true  -> Line Voltage detected
          false -> Line Voltage not detected
*/
...
boolean LineVoltage_beforeOn_Test (uint8_t line1,uint8_t line2,uint8_t line3){
  return LineVoltage_beforeOn_Test(line1 && line2 && line3);
}
boolean LineVoltage_beforeOff_Test (uint8_t line1,uint8_t line2,uint8_t line3){
  return LineVoltage_beforeOff_Test(line1 || line2 || line3);
}
...
void read_Meter(float * output_V,float * output_freq) {
  DLT645_init();
	...
	if(DLT645_read(sizeof_PHASE)) {
		// chk++;
		*output_V     = (((uint32_t)DLT645_Data[17]<< 8) | ((uint32_t)DLT645_Data[16])) /100.0f;
		...
		*output_freq   = (((uint32_t)DLT645_Data[35]<< 8) | ((uint32_t)DLT645_Data[34]))/100.0f;
